...
export default function ReferenceWorkoutPreview({db={},activeSession,items=[],onStart,onLibrary}){
  ...
  return <div className="reference-workout-v3" data-testid="reference-workout-preview">
    ...
    <section className="ref3-exercises"><h2>EXERCÍCIOS</h2><div>{safeItems.map((x,i)=>{const item=x||{},ex=getExercise(db,item.exercise_id),exName=ex.name||item.name||item.exercise_id||`Exercício ${i+1}`,equipment=asArray(ex.equipment??item.equipment),artExercise={...item,...ex,equipment,name:exName,id:ex.id||item.exercise_id};return <article key={`${item.exercise_id||exName}-${i}`}><ExercisePhoto exercise={artExercise}/><div className="ref3-ex-copy"><strong>{exName}</strong><span>{item.sets??"-"} séries x {item.reps??"-"} · RPE {toRpe(item.rir)}</span></div><i className="ref3-check"><Check size={19}/></i></article>})}</div></section>
