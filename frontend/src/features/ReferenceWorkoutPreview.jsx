...
const ExerciseArtwork=({slot,label})=>{const style=slot>=0?{"--art-y":`${slot*-82}px`,"--art-y-small":`${slot*-72}px`,backgroundImage:`url(${EXERCISE_ARTWORK_SPRITE})`}:undefined;return <div className={`ref3-ex-art${slot<0?" fallback":""}`} style={style} role="img" aria-label={label} data-art-slot={slot}>{slot<0&&<Dumbbell size={27}/>}</div>};
