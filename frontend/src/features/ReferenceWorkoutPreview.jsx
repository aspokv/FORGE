...
const ExerciseArtwork=({slot,label,src})=>{
  const [failed,setFailed]=useState(false);
  const valid=(Boolean(src)||slot>=0)&&!failed;
  const style=valid&&!src?{"--art-y":`${slot*-82}px`,"--art-y-small":`${slot*-72}px`}:undefined;
  return <div className={`ref3-ex-art${valid?"":" fallback"}`} role="img" aria-label={label} data-art-slot={slot}>
    {valid?<img className={src?"ref3-reviewed-art":undefined} src={src||ART_SPRITE_SRC} alt="" aria-hidden="true" style={style} onError={()=>setFailed(true)}/>:<Dumbbell size={27}/>}
  </div>
};
...
export default function ReferenceWorkoutPreview({db={},activeSession,items=[],onStart,onLibrary}){
  ...
  return <div className="reference-workout-v3" data-testid="reference-workout-preview">
    ...
    <section className="ref3-exercises"><h2>EXERCÍCIOS</h2><div>{safeItems.map((x,i)=>{const item=x||{},ex=getExercise(db,item.exercise_id),exName=ex.name||item.name||item.exercise_id||`Exercício ${i+1}`,equipment=asArray(ex.equipment??item.equipment),artExercise={...item,...ex,equipment,name:exName,id:ex.id||item.exercise_id},slot=artworkSlotForExercise(artExercise),src=reviewedArtworkForExercise(artExercise);return <article key={`${item.exercise_id||exName}-${i}`}><ExerciseArtwork key={src||slot} src={src} slot={slot} label={`Ilustração de ${exName}`}/><div className="ref3-ex-copy"><strong>{exName}</strong><span>{item.sets??"-"} séries x {item.reps??"-"}</span></div><em>RPE {toRpe(item.rir)}</em><i className="ref3-check"><Check size={19}/></i></article>})}</div></section>
