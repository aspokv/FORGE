...
import {nextSessionAfter} from "./workoutCompletionState";
 ...
 return <AstraPage screen={1} testId="workout-completed-today" onLibrary={onLibrary}>
  ...
  <section className="a6-panel completed-workout-card">
   ...
   <details className="a6-details"><summary>Ver resumo da sessão</summary>
    ...
    {completed?<><p>Exercícios previstos para a sessão:</p><ul>{(completed.exercises||[]).map((x,i)=><li key={i}>{nameOf(x)}</li>)}</ul></>:<p>Os exercícios desta sessão não estão no programa atual.</p>}
   </details>
  </section>
  {next?<section className="a6-panel completed-next-card">
    <div className="completed-next-heading"><div><p className="a6-eyebrow">PRÓXIMO TREINO</p><h2>{next.label||"Próxima sessão"}</h2><p>{(next.focus||[]).join(" · ")}</p></div><TrainingCardImage session={next} program={program} profile={db.profile} width="120" height="100"/></div>
