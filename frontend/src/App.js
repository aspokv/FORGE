...
function Profile({db,redo,openBuilder,openManual,signOut,user,openAnalysis,openPlans}){
  ...
  return <div className="content profile-page">
    ...
    <section className="panel"><p className="eyebrow">AÇÕES</p><div className="action-row">{openPlans&&<button className="secondary-button"data-testid="open-plans-button"onClick={openPlans}><ShieldCheck size={16}/> Meu plano e assinatura</button>}{openAnalysis&&<button className="secondary-button"data-testid="open-analysis-button"onClick={openAnalysis}><BarChart3 size={16}/> Análise da semana</button>}<button className="secondary-button"data-testid="redo-assessment-button"onClick={redo}>Refazer avaliação</button><button className="secondary-button"data-testid="open-builder-button"onClick={openBuilder}><Sliders size={16}/> {manual?"Editar programa manual":"Program Builder Pro"}</button><button className="secondary-button"data-testid="open-manual-button"onClick={openManual}><FileUp size={16}/> Criar meu próprio treino</button><button className="secondary-button"data-testid="visual-assessment-button"onClick={()=>setVisual(!visual)}><FileUp size={16}/> Analisar meu físico</button>{signOut&&<button className="secondary-button"data-testid="signout-button"onClick={signOut}><LogOut size={16}/> Sair da conta</button>}</div>
