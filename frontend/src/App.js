...
function Profile({db,redo,openBuilder,openManual,signOut,user,openAnalysis,openPlans}){
  ...
  return <div className="content profile-page">
    <div className="section-intro"><p className="eyebrow">SEU PERFIL</p><h2>{db.profile.name}</h2><p className="muted">{db.profile.experience} · {db.profile.goal} · {db.profile.days} dias por ciclo</p></div>
    <section className="panel profile-panel"><div className="avatar">{(db.profile.name||"AF").split(" ").map(x=>x[0]).slice(0,2).join("")}</div><div><p className="eyebrow">CONFIGURAÇÃO ATUAL</p><h3>{db.profile.session_minutes} min · {manual?"Programação manual":rotuloDoModo(db.profile.automation_mode)||"Programação assistida"}</h3><p className="muted">{user?`${user.email}`:"Preferências salvas"}</p>{user&&<p className="fg-selos"><span className="fg-selo">{rotuloDoPlano(user.plan)||"Plano"}</span><span className={user.status==="ACTIVE"?"fg-selo ok":"fg-selo"}>{rotuloDaSituacao(user.status)||"—"}</span></p>}</div></section>
    <section className="panel training-preferences"data-testid="training-preferences"><div className="panel-top"><div><p className="eyebrow">ARQUITETURA DO TREINO</p><h3>Divisão e método FORGE</h3></div><span className="live-pill"><i/> AJUSTE AUTOMÁTICO</span></div><p className="muted">Você escolhe a preferência; o FORGE limita as opções ao que cabe nos seus dias e na sua recuperação.</p>
    ...
    <section className="panel">{/* Tres pesos, e nao sete botoes iguais. Ir para outra tela, mudar o proprio treino e    sair da conta sao coisas diferentes, e antes tinham o mesmo desenho — inclusive    "Sair da conta", ao lado de "Criar meu proprio treino". */}<p className="eyebrow">SUA CONTA</p><div className="fg-lista-acoes">{openPlans&&<button className="fg-acao-linha"data-testid="open-plans-button"onClick={openPlans}><ShieldCheck size={17}/><span>Meu plano e assinatura</span><ChevronRight size={17}/></button>}{openAnalysis&&<button className="fg-acao-linha"data-testid="open-analysis-button"onClick={openAnalysis}><BarChart3 size={17}/><span>Análise da semana</span><ChevronRight size={17}/></button>}</div><p className="eyebrow"style={{marginTop:20}}>SEU TREINO</p><div className="fg-lista-acoes"><button className="fg-acao-linha"data-testid="redo-assessment-button"onClick={redo}><RotateCcw size={17}/><span>Refazer avaliação</span><ChevronRight size={17}/></button><button className="fg-acao-linha"data-testid="open-builder-button"onClick={openBuilder}><Sliders size={17}/><span>{manual?"Editar programa manual":"Montar programa avançado"}</span><ChevronRight size={17}/></button><button className="fg-acao-linha"data-testid="open-manual-button"onClick={openManual}><FileUp size={17}/><span>Criar meu próprio treino</span><ChevronRight size={17}/></button><button className="fg-acao-linha"data-testid="visual-assessment-button"onClick={()=>setVisual(!visual)}><Activity size={17}/><span>Analisar meu físico</span><ChevronRight size={17}/></button></div>{signOut&&<button className="fg-btn fg-btn-3 fg-sair"data-testid="signout-button"onClick={signOut}><LogOut size={16}/> Sair da conta</button>}
...
function ProgramPreview({program,onApprove,onBack}){const p=program||{};return <div className="onboarding deep-scene"><div className="onboard-scene"><p className="eyebrow">PREVIEW DO PROGRAMA</p><h1>Seu programa FORGE ASSISTED</h1><p className="onboard-copy">{p.name||"Programa adaptativo"} · {p.week||""}</p></div><section className="panel"><div className="panel-top"><div><p className="eyebrow">LÓGICA</p><h3>{p.logic?.split||p.name}</h3></div></div><p className="muted">{p.logic?.days||0} sessões · volume ×{p.logic?.recovery_modifier||1}. Prioridade manual preservada.</p></section>{(p.sessions||[]).map((s,i)=><section className="panel"key={i}><p className="eyebrow">DIA {s.day} · {s.label} · {s.demand}</p><div className="focus-row">{(s.focus||[]).map(f=><span key={f}>{f}</span>)}</div>{(s.exercises||[]).map((x,j)=><div className="muscle-row program-preview-exercise"key={j}><ExercisePhoto exercise={x}/><div><b>{x.exercise_id}</b><p>{x.sets}×{x.reps} RIR {x.rir} · {x.rest}{x.technique_id!=="straight"?` · ${x.technique}`:""}</p></div></div>)}</section>)}<div className="deep-actions"style={{marginTop:28}}><button className="secondary-button"data-testid="preview-back-button"onClick={onBack}>Voltar e ajustar</button><button className="primary-button"data-testid="approve-program-button"onClick={onApprove}><Check size={18}/> Aprovar programa</button></div></div>}
