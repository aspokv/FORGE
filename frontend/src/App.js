...
function Page({tab,db,analytics,report,techniques,start,openCoach,openBuilder,openManual,onLibraryBuild,openTech,redo,signOut,user,goHome,onExerciseSubstituted,onWorkoutCompleted,onRecoveryCheckin}){if(tab==="Treino")return <Workout db={db}techniques={techniques}openTech={openTech}goHome={goHome}onExerciseSubstituted={onExerciseSubstituted}onWorkoutCompleted={onWorkoutCompleted}onLibraryBuild={onLibraryBuild}/>;if(tab==="Planos")return <Billing API={API}/>;if(tab==="Progresso")return <Progress analytics={analytics}profileId={db?.profile?.id}/>;if(tab==="Análise")return <Analysis db={db}analytics={analytics}report={report}openCoach={openCoach}/>;if(tab==="Perfil")return <Profile db={db}redo={redo}openBuilder={openBuilder}openManual={openManual}signOut={signOut}user={user}/>;if(tab==="Alimentação")return <Nutrition db={db}API={API}profileId={db?.profile?.id}/>;return <Today db={db}analytics={analytics}report={report}start={start}openBuilder={openBuilder}openManual={openManual}onRecoveryCheckin={onRecoveryCheckin}/>}
...
function Today({db,analytics,report,start,openBuilder,openManual,onRecoveryCheckin}){
  ...
  return <div className="content performance-home forge-home-final decision-home ledger-home">
    ...
    <section className={`today-action-bar${checkin?" checked":""}`} data-testid="daily-briefing">
...
function Workout({db,techniques,openTech,goHome,onExerciseSubstituted,onWorkoutCompleted,onLibraryBuild}){
  const p=db.program||{};
  const activeSession=p.sessions?.find(s=>s.day===p.active_day)||p.sessions?.[0];
  ...
  if(view==="library")return <div className="training-library-view"data-testid="training-library-view">
    ...
    <WorkoutLibrary API={API}exercises={db.exercises||[]}onBuild={onLibraryBuild}/>
  </div>;
