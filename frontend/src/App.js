...
import {Activity,BarChart3,Bell,BookOpen,BrainCircuit,Check,ChevronRight,CircleUserRound,Droplets,Dumbbell,FileUp,Home,Info,LineChart,LockKeyhole,LogOut,RotateCcw,ShieldCheck,Sliders,Sparkles,TimerReset,TrendingUp,Trophy,UserRound,Utensils,X} from "lucide-react";
import "@/App.css";
...
import WorkoutLibrary,{templateToSession} from "./features/WorkoutLibrary";
...
function Page({tab,db,analytics,report,techniques,start,openCoach,openBuilder,openManual,onLibraryBuild,onLibraryTemplateAdd,openTech,redo,signOut,user,goHome,onExerciseSubstituted,onWorkoutCompleted,onRecoveryCheckin}){if(tab==="Treino")return <Workout db={db}techniques={techniques}openTech={openTech}goHome={goHome}onExerciseSubstituted={onExerciseSubstituted}onWorkoutCompleted={onWorkoutCompleted}onLibraryBuild={onLibraryBuild}onLibraryTemplateAdd={onLibraryTemplateAdd}/>;if(tab==="Planos")return <Billing API={API}/>;if(tab==="Progresso")return <Progress analytics={analytics}profileId={db?.profile?.id}/>;if(tab==="Análise")return <Analysis db={db}analytics={analytics}report={report}openCoach={openCoach}/>;if(tab==="Perfil")return <Profile db={db}redo={redo}openBuilder={openBuilder}openManual={openManual}signOut={signOut}user={user}/>;if(tab==="Alimentação")return <Nutrition db={db}API={API}profileId={db?.profile?.id}/>;return <Today db={db}analytics={analytics}report={report}start={start}openCoach={openCoach}openBuilder={openBuilder}openManual={openManual}onRecoveryCheckin={onRecoveryCheckin}/>}
...
function Today({db,analytics,report,start,openCoach,openBuilder,openManual,onRecoveryCheckin}){
  ...
  return <div className="content performance-home forge-home-final decision-home ledger-home">
    ...
    <section className="home-coach-pro" data-testid="home-coach-pro">
      <div className="home-coach-orbit"><BrainCircuit size={32}/><i/><i/><i/></div>
      <div><p>COACH IA PRO · INCLUÍDO NO SEU PLANO</p><h3>{checkin?"Seu plano está ajustado à sua recuperação.":"Seu próximo nível começa agora."}</h3><span>Treino, nutrição e progressão com o contexto completo do seu ciclo.</span></div>
      <button type="button" onClick={openCoach}>Falar com o Coach <ChevronRight size={16}/></button>
    </section>
