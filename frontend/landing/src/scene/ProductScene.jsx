...
import {AdaptiveDpr,Environment,Lightformer,ContactShadows,Html,useGLTF,useTexture,PerformanceMonitor} from '@react-three/drei';
...
function Device({progress,finish,rotation,autoRotate,reduced,onHotspot,mobile}){
 ...
 useEffect(()=>{Object.values(textures).forEach(t=>{t.colorSpace=SRGBColorSpace;t.anisotropy=Math.min(mobile?4:8,gl.capabilities.getMaxAnisotropy());t.magFilter=LinearFilter;t.minFilter=LinearMipmapLinearFilter;t.generateMipmaps=true;t.needsUpdate=true;})},[textures,gl,mobile]);
 ...
 useFrame(({camera,clock},delta)=>{
  ...
  group.current.rotation.y=MathUtils.damp(group.current.rotation.y,-.30+Math.sin(p*Math.PI*2)*.36+rotation.current.x+screenBlend(p).turn,5,dt);
 ...
 return <group ref={group} scale={mobile?.91:1}>
...
export default function ProductScene(props){
 const [dpr,setDpr]=useState(props.mobile?1:1.5);
 return <Canvas frameloop={props.active?'always':'never'} shadows={!props.mobile} dpr={dpr} camera={{position:[0,0,8.2],fov:38}} gl={{antialias:true,alpha:true,powerPreference:'high-performance'}} onCreated={({gl})=>{gl.domElement.addEventListener('webglcontextlost',e=>{e.preventDefault();props.onFailure()},{once:true})}}>
  ...
  <Device {...props}/>{!props.mobile&&<ContactShadows position={[0,-2.75,0]} opacity={.28} scale={12} blur={2.5} far={6} resolution={256} frames={1}/>}</Suspense>
